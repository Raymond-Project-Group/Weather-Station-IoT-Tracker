...
static int32_t gps_uart_worker(void* context) {
    ...
    while(1) {
        uint32_t events = furi_thread_flags_wait(
            GPS_UART_WORKER_ALL_RX_EVENTS, FuriFlagWaitAny, FuriWaitForever);
...
void gps_uart_deinit_thread(GpsUart* gps_uart) {
    furi_assert(gps_uart);
    furi_thread_flags_set(furi_thread_get_id(gps_uart->thread), GpsUartWorkerEvtStop);
