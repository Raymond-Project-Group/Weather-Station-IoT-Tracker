...
void logger_stream_append(App* app) {
    ...
    append_bme_log(app->file_stream, app->bme280->data, !app->bme_initialized);
    stream_write_char(app->file_stream, ',');
    append_gps_log(app->file_stream, app->gps_uart->status, !app->gps_initialized);
    stream_write_char(app->file_stream, ',');
...
void logger_auto_append(void* context) {
    furi_assert(context);
    App* app = (App*) context;
    FURI_LOG_I(TAG,"Appending Context");
