...
void* ws_protocol_encoder_acurite_606tx_alloc(SubGhzEnvironment* environment) {
    ...
    instance->generic.data_count_bit = ws_protocol_acurite_606tx_const.min_count_bit_for_found;
    instance->encoder.repeat = 7; //dont repeat, only send once
    instance->encoder.size_upload = 80;
...
static bool ws_protocol_encoder_acurite_606tx_get_upload(WSProtocolEncoderAcurite_606TX* instance) {
    ...
    for(uint8_t i = instance->generic.data_count_bit; i > 0; i--) {
        if(bit_read(instance->generic.data, i - 1)) {
            ...
            instance->encoder.upload[index++] = level_duration_make(false, (uint32_t)ws_protocol_acurite_606tx_const.te_long*2);
        } 
        else {
            //send bit 0
    ...
    //Maybe a Trailer bit to signal end
    instance->encoder.upload[index++] = level_duration_make(true, (uint32_t)ws_protocol_acurite_606tx_const.te_short);
...
void ws_protocol_acurite_606tx_reconstructor(WSBlockGeneric* instance) { //added for encoding
    uint64_t data;
    data = instance->id; //id added to data
    ...
    uint16_t temp_raw;
    if(instance->temp<0){//if temp is negative
        data=(data<<1) | 1;
        temp_raw = (int)(instance->temp*-10);
        temp_raw = ~temp_raw; //inverted
        temp_raw +=1; //add one
    }
    else{
        data=(data<<1);
    ...
    temp_raw = temp_raw & 0x7FF;//11 bits
    data = (data<<11) | temp_raw;
    ...
    uint8_t reg = 0x7C;
    uint8_t temp_reg = 0;
    uint8_t LSFR_sequence[32] = {0};//size is min_count_bit_for_found
    for (int i = 0; i < ws_protocol_acurite_606tx_const.min_count_bit_for_found; i++) {
        temp_reg = reg & 0x01;
        reg >>= 1;
        reg |= (temp_reg << 7);
    ...
    for (int byte_idx = 0; byte_idx < 3; byte_idx++) {
        for (int bit_idx = 7; bit_idx >= 0; bit_idx--) {
            bit = (msg[byte_idx] & (1 << bit_idx)) >> bit_idx;
            if (bit) {
                hash_reg ^= LSFR_sequence[byte_idx * 8 + (7 - bit_idx) + 4];
    ...
    FURI_LOG_I("LSFR Reconstructing","%x",hash_reg);
    data = (data<<8) | hash_reg; //added checksum to data
